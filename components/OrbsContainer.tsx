...
function OrbsContainer({ isMobile, isMidsize, scroll }: OrbsContainerProps) {
  ...
  // While isRunning, orbs are added to the orbs array every 2.5 seconds. If the amount of orbs
  // reaches 15, the oldest orb is removed and the newest orb is added.
  useEffect(() => {
    if (isRunning) {
      if (orbs.length >= 2) {
        setOrbTimer(2500);
  ...
  // Click handler for the add orb button. When the amount of orbs reaches 20, the 
  // oldest orb is removed and the newest orb is added.
  ...
  // Click handler for the clear orbs button. All orbs are removed from the orbs array.
